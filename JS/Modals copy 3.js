...
function showPreImpModal(itemId) {
    ...
    _modalBody = '<div class="col-md-8">' +
        ...
        '</div>' +
        '<div class="col-md-3">'+
        '<input type="time" class="form-control" id="idStartTime">'+
        '</div>'+
        '</div><br>' +
        '<div class="row">' +
        '<div class="col-md-3">' +
        ...
        '</div>' +
        // '<div class="col-md-3">'+
        // '<input type="time" class="form-control" id="idEndTime">'+
        // '</div>'+
        '</div><br>' +
        '<div class="row">' +
        '<div class="col-md-3">' +
...
function showImplementationModal(itemId) {
    ...
    openModal('#tblPreImp', 'modalIMP', '#tbodyImp', _modalHeader, _modalBody, _modalFooter, itemId);
...
function showPostImpModal(itemId) {
    ...
    _modalBody = '<div class="col-md-8">' +
        ...
        '</div>' +
        '<div class="col-md-4">' +
        '<input type="datetime-local" class="form-control" id="idEndDatePost">' +
        '</div>' +
        // '<div class="col-md-3">'+
        // '<input type="time" class="form-control" id="idEndTime">'+
        // '</div>'+
    ...
    openModal('#tblPreImp', 'modalIMP', '#tbodyPostImp', _modalHeader, _modalBody, _modalFooter, itemId);
}
...
function showImpactModal(itemId) {
    ...
    openModal('#tblPreImp', 'modalIMP', '#tbodyImpact', _modalHeader, _modalBody, _modalFooter, itemId);
}
    ...
    if(listName == "Impact Assessment Test"){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                debugger;
                if(data.d.results.length != 0){
                    $("#modalIMP #idStartDate").val((item[0].Start_x0020_Date_x0020_and_x0020).split('T')[0]);
                    $("#modalIMP #idEndDate").val((item[0].End_x0020_Date_x0020_and_x0020_T).split("T")[0]);
                    $("#modalIMP #idActionTaken").val(item[0].Implementer);
                    $("#modalIMP #idImplementer").val(item[0].Title);
                }else{
                    alert('Value cannot be found')
                }
            },
...
function PreImpSave(id) {
    ...
    if(id!=0){
        SaveItem(listName, dataArrPreImp, id);//update function
    }else{
        SaveItem(listName, dataArrPreImp, 0);//new item to save
    }
}
