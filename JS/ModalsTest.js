    ...
    if(listName == "Impact Assessment Test"){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                console.log("(1) This is Item ID: ",itemId);
                // console.log(item);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                // console.log("Test PRinting values fro update buttons");
                // console.log(item[0].Start_x0020_Date_x0020_and_x0020);
                // console.log(item[0].End_x0020_Date_x0020_and_x0020_T);
                // console.log(item[0].Title);
                // console.log(item[0].Implementer);
                console.log(item);
                if(data.d.results.length != 0){
...
function addItems(listName, v, id) {
    ...
    var tr = "<tr id='{5}'><td>{0}</td><td>{1}</td><td> {2}</td><td> {3}</td><td><a class='waves-effect waves-light' onclick=\"editItem({4}, '{5}', '{6}')\"><i class='far fa-edit'></i></a></td><td><a class='waves-effect waves-light' onclick=\"deleteCon({4},'{5}')\"><i class='far fa-trash-alt'></i></a></td></tr>";
    tr = tr.format([v.Start_x0020_Date_x0020_and_x0020, v.End_x0020_Date_x0020_and_x0020_T, v.Title, v.Implementer, id, idname, cListname]);
    // debugger;
    $(cTbody).append(tr);
    $('#' + cmodalID).modal('hide');
...
function addItemsMitigation(listName, v, id) {
    ...
    $(cTable).removeClass("hidden");
    var tr = "<tr id='{4}'><td>{0}</td><td>{1}</td><td> {2}</td><td><a class='waves-effect waves-light' onclick=\"editItem({3}, '{4}', '{5}')\"><i class='far fa-edit'></i></a></td><td><a class='waves-effect waves-light' onclick=\"deleteCon({3},'{4}')\"><i class='far fa-trash-alt'></i></a></td></tr>";
    tr = tr.format([v.Title, v.Scale, v.Mitigation_x0020_Actions, id, idname, cListname]);
    //alert(tr);
    $(cTbody).append(tr);
    $('#' + cmodalID).modal('hide');
    //$(cTable).closest('div.container').addClass('show');
