...
function showPreImpModal(itemId) {
    ...
    _modalFooter = '<a class="btn btn-primary" onclick="PreImpSave(' + itemId + ');"> Save </a> <a class="btn btn-primary" data-dismiss="modal">Close</a>';
...
function showImplementationModal(itemId) {
    ...
    _modalBody = '<div class="col-md-8">' +
        ...
        '</div>' +
        '</div>';
    _modalFooter = '<a class="btn btn-primary" onclick="ImpSave(' + itemId + ');"> Save </a> <a class="btn btn-primary" data-dismiss="modal">Close</a>';
...
function showPostImpModal(itemId) {
    ...
    _modalBody = '<div class="col-md-8">' +
        ...
        '</div>' +
        '</div>';
    _modalFooter = '<a class="btn btn-primary" onclick="PostImpSave(' + itemId + ');"> Save </a> <a class="btn btn-primary" data-dismiss="modal">Close</a>';
    openModal('#tblPostImp', 'modalIMP', '#tbodyPostImp', _modalHeader, _modalBody, _modalFooter, itemId);
}

function showImpactModal(itemId) {
    // alert('test');
    listName = 'Impact Assessment';
    ...
    _modalBody = '<div class="col-md-8">' +
                        ...
                        '<div class="row">' +
                            ...
                            '</div>' +
                            '<div class="col-md-4">' +
                                '<input type="number" min="1" max="5" class="form-control" id="idImpactScale">' +
                            '</div>' +
                        '</div><br>' +
                        '<div class="row">' +
                            '<div class="col-md-3">' +
                            ...
                            '</div>' +
                            '<div class="col-md-7">' +
                                '<input rows="4" class="form-control" id="idImpactMitigationAction">' +
                            '</div>' +
        '</div>';

    _modalFooter = '<a class="btn btn-primary" onclick="ImpactSave(' + itemId + ');"> Save </a> <a class="btn btn-primary" data-dismiss="modal">Close</a>';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.d.results.length != 0) {
                    ...
                    console.log('no Mitigation attached to this item.')
                }
            },
            error: function (data) {
...
function ImpactSave(id) {
    ...
    var ImpactDescription = $("#modalIMP #idImpactDesc").val();
    var ImpactScale = $("#modalIMP #idImpactScale").val();
    var ImpactMitigations = $("#modalIMP #idImpactMitigationAction").val();
...
function onQueryFailedModal(sender, args) {
    console.log('Failed: \n' + args.get_message() + '\n' + args.get_stackTrace());
    debugger;
    console.log(args);
    console.log("Save as draft failed.");
}
...
function addItems(listName, v, id) {
    ...
    var sdate = v.Start_x0020_Date_x0020_and_x0020.split("T")[0];
    var stime = v.Start_x0020_Date_x0020_and_x0020.split("T")[1];
    var edate = v.End_x0020_Date_x0020_and_x0020_T.split("T")[0];
    var etime = v.End_x0020_Date_x0020_and_x0020_T.split("T")[1];
    tr = tr.format([sdate + " " + stime,
        edate + " " + etime,
        v.Title,
        v.Implementer,
        id,
...
function addItemsMitigation(listName, v, id) {
    ...
    $(cTable).removeClass("hidden");
    var tr = "<tr id='{4}'><td style='text-left'>{0}</td>" +
        "<td>{1}</td>" +
        "<td style='text-left'>{2}</td>" +
...
function editItem(id, trID, listName) {
    ...
    if (listName == "Pre Implementation") {
        showPreImpModal(id);
    } else if (listName == "Implementation") {
        showImplementationModal(id);
    } else if (listName == "Post Implementation") {
        showPostImpModal(id);
    } else if (listName == "Impact Assessment") {
        showImpactModal(id);
    } else {
...
function deleteCon(id, tdId) {
    ...
    _modalFooter = "<a class='btn btn-primary' onclick=\"deleteItem({0},'{1}','{2}')\"> Yes </a> <a class='btn btn-primary' data-dismiss='modal'>No</a>";
    _modalFooter = _modalFooter.format([id, "modalDelete", tdId]);
    openModal('', 'modalDelete', '', _modalHeader, _modalBody, _modalFooter, id);
    debugger;
}
